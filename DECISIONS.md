...
| ADR-0024 | QROS Control Center Stage 2 — wired pipeline (Telegram polling + OpenAI + webhook + parser + AgentOS forwarding + health + logs + graceful) | Wire existing Stage 1 structure; no redesign, no Redis/Dashboard | 2026-09-15 (Control Center Stage 2, MISSION-003) |
| ADR-0023 | QROS Control Center Stage 1 — remote project management structure (Bot+Gateway+Watcher, compose+env+docs, HMAC, allowlist, 17 checks PASS, no trading/AgentOS mutation) | Control Center as client of AgentOS via GitHub; Stage 2 will wire polling/OpenAI/GitHub forward | 2026-09-15 (Control Center Stage 1, MISSION-001) |
| ADR-0022 | Market Digital Twin — single source of truth simulator (TwinTypes/Clock/Bus/Validator/Core+Adapters, FNV-1a exact, 5 consumers identical via TwinEventBus) | Single timeline, hash proof, no consumer bypass; Replay/Training/Risk/Research/AI receive same SMarketEvent | 2026-09-15 (Twin, TASK-0005) |
| ADR-0021 | AgentOS — Git-native multi-agent collaboration layer (8-file bus, 10 workers 1:1, CLI/validate/tests, Git-only, no Telegram) | Autonomous collaboration without direct messaging; all comms Git-based; next AgentOS improvements | 2026-09-15 (AgentOS v1.0) |
