...
| **Module Status** | 54 `.mqh` + 1 `.mq5` (EA build unit: +CAIRiskAdvisory) + 7 isolated validation scripts + 1 risk advisory test, compile pending MT5 verification (Linux sandbox no MetaEditor); trading modules frozen; trading modules frozen; research/replay/history dormant at init |
...
| Advisory Risk AI | `EAContext/` (6) — CAIRiskAdvisory | **PASS** — shadow-only, dormant OFF, no trading authority (ADR-0016) |
